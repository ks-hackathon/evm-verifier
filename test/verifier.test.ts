...
describe("evm-verifier", function () {
  ...
  it("mint a sbt", async function () {
    const contractFactory0 = await ethers.getContractFactory("RingSigVerifier");
    const SigVerifier = await contractFactory0.deploy();

    // link libraries
    const contractFactory = await ethers.getContractFactory("AliceRingToken");

    // deploy SigVerifier contract
    const aliceRingToken = await contractFactory.deploy(SigVerifier);
    ...
    console.log((await aliceRingToken.connect(signer[0]).mint(
      "0x4838B106FCe9647Bdf1E7877BF73cE8B0BAD5f97" as AddressLike,
      0n as BigNumberish,
      "uri",
      "proofId",
      message,
    ...
    // check balance
    expect(Number(await aliceRingToken.balanceOf(signer[0].address))).to.equal(1);
